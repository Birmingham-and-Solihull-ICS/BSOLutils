...
MI2 %>%
  ...
  labs(colour = "Deprivation Qunitile",
       ...
       title = "Age-standardised Admission Ratio: Myocardial Infarction (Under 75yrs)",
...
MI2$age_group_code <- factor(MI2$age_group_code)
MI2$start_date <- factor(MI2$start_date)

MI_model1 <- glm.nb(numerator ~ 1 +
                   #imd_grp +
                   age_group_code +
                   start_date +
...
summary(MI_model1)
exp(coef(MI_model1))
exp(confint(MI_model1))

disp_ratio(MI_model1)
...
phe <- calculate_ISRatio(MI2, x=numerator, n=denominator, x_ref = grp_num, n_ref  = grp_denom,
                         refpoptype = "field")
...
MI2$x <- MI2$numerator
MI2$n <- MI2$denominator
MI2$x_ref <- MI2$grp_num
MI2$n_ref <- MI2$grp_denom
...
b %>%
  ...
  theme(axis.text.x = element_text(angle = 45),
        plot.subtitle = element_text(face = "italic"),
        legend.position = "right",# legend.position.inside = c(0.85,0.95),
        legend.background = element_rect(colour = "#FFF", color = "#000"))
